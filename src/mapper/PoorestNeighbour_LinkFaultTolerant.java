    ...
    public static Topology generateTopology(Graph graph) {
        ...
        while (itr.hasNext()) {
            ...
            while (itr2.hasNext()) {
                ...
                if (b > a) {//to guarantee that each link is visited only once
        ...
        for (Map.Entry entry : newLinkages.entrySet()) {
            ...
            Edge edge = new Edge(joinee, poorestNeighbour);
            if (!topology.connections.get(joinee).contains(poorestNeighbour)) topology.addConnection(edge);
        }
        return topology;
    }
